...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (!token) return next(new AppError('You are not logged in! ' +
    'Please log in to get access.', 401));
  // Verification token
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  const message = `Forgot password? Submit a PATCH request 
  ...
  try {
    ...
    return next(new AppError('There was a problem with sending this email. ' +
      'Try again later!', 500));
  }
});
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  // Set the new password if token has not expired and there is user
  if (!user) {
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  if (!(await user.correctPassword(passwordCurrent, user.password))) {
    return next(new AppError('Your current password is wrong. Please log in again', 401));
  }
